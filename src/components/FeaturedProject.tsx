...
type LinkDescriptor = { name: string, href: string, icon: React.FC<HTMLAttributes<SVGSVGElement>> };

export interface FeaturedProjectProos {
    name: string;
    description: string;
    href: string;
...
export const FeaturedProject: React.VFC<FeaturedProjectProos> = function (props) {
	const {
		...
		<div className={`relative flex flex-wrap md:flex-nowrap ${alignRight ? "flex-row" : "flex-row-reverse"}`}>
			<Link href={href}>
				<a target="_blank" className="relative w-fit md:w-[32rem] lg:w-[48rem] xl:w-[64rem] group grow-0 h-fit mb-4 mr-4    ">
					...
					<Image className="rounded filter grayscale contrast-100 brightness-90 group-hover:brightness-100 group-hover:grayscale-0" src={image} alt={`Screenshot of ${name}`} quality={100} placeholder="blur" />
				</a>
			...
			<div className={`flex flex-col w-full mt-8 md:mt-0 space-y-4 ${alignRight ? "text-right md:-ml-16" : "text-left md:-mr-16"}`}>
				...
				<div className={`z-30 p-6 rounded bg-neutral-800 text-neutral-400 shadow-2xl border-red-400 ${alignRight ? "border-r-2" : "border-l-2"}`}>
					{description}
				</div>
				<div className={`flex flex-col space-y-4 ${alignRight ? "md:ml-24" : "md:mr-24"}`}>
					<div className="flex font-mono text-sm text-neutral-400">
					...
					<div className={`flex gap-x-4 ${alignRight ? "flex-row-reverse" : "flex-row"}`}>
						{[{ name, href, icon: IconExternalLink }, ...links].map((descriptor) => {
